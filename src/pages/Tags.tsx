...
const Tags = () => {
  ...
  return (
    <div className="min-h-screen bg-deep-olive">
      ...
      <header ref={headerRef} className="pt-20 pb-8 px-6 opacity-0 transition-opacity duration-500">
        <div className="max-w-wide mx-auto">
          <h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-bold mb-6 text-center text-cream">
            <Link to="/tags" onClick={() => { scrollToTop(); setSelectedTags([]); }} className="text-olive-light hover:text-tomato transition-colors">Tags</Link>
      ...
      <footer className="px-6 py-8 border-t border-moss">
        <div className="max-w-wide mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <Link to="/" onClick={scrollToTop} className="font-serif text-lg font-semibold text-cream hover:text-tomato transition-colors">George</Link>
          <div className="flex items-center gap-6 text-sm text-earth-tan">
            <Link to="/" onClick={scrollToTop} className="hover:text-cream transition-colors">Home</Link>
            <Link to="/tags" onClick={scrollToTop} className="hover:text-cream transition-colors">Tags</Link>
            <Link to="/projects" onClick={scrollToTop} className="hover:text-cream transition-colors">Projects</Link>
            <Link to="/about" onClick={scrollToTop} className="hover:text-cream transition-colors">About</Link>
          </div>
        </div>
      </footer>
    </div>
